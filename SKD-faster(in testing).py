...
import pickle
from vllm import LLM, SamplingParams  # For faster batched generation

# Enable memory-efficient SDP for faster attention on H100 (PyTorch >=2.1)
...
# Load vLLM for fast batched generation (optimized for H100)
teacher_generator = LLM(
    model=LLAMA_TEACHER,
    dtype="bfloat16",
    tensor_parallel_size=1,  # Single GPU (H100)
    gpu_memory_utilization=0.9  # High utilization for speed
)

# Load HF teacher model for logits computation (vLLM doesn't expose logits easily)
...
def aggregate_persona_responses(question, options):
    prompts = []
    role_specs = list(role_specifications.values())
    for spec in role_specs:
        prompt = spec + f"\n\nQuestion: {question}\n\nOptions:\n" + "\n".join(f"{i}: {opt}" for i, opt in enumerate(options)) + "\nAnswer:"
        prompts.append(prompt)
    
    # Use vLLM for batched, fast generation
    sampling_params = SamplingParams(
        max_tokens=100,  # Reduced for speed
        temperature=0.0,  # Greedy (matches do_sample=False)
        top_p=1.0
    )
    outputs = teacher_generator.generate(prompts, sampling_params)
...
class DistillationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        item = self.data[idx]
        # Concatenate input + teacher_response for consistent full-sequence handling
        full_text = item["input"] + item["teacher_response"]
        ...
        return {
            ...
            "labels": encoding["input_ids"].squeeze(0)  # Use full sequence as labels (shifted in loss if needed)
        }
...
def process_dataset():
    ...
    dataset = dataset.shuffle(seed=SEED).select(range(500))  # For testing: change to range(100) for ~5-10 min run
    ...
    for ex in tqdm(dataset, desc="Processing examples"):
        ...
        # Truncate aggregated response early for speed
        teacher_response = tokenizer.decode(tokenizer.encode(teacher_response, max_length=256, truncation=True))
