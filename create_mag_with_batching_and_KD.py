...
def distill_final_token_loss(student_logits, teacher_logits, labels, T=2.0,
                              ce_weight=0.5, kl_weight=0.4, cosine_weight=0.1):
    student_final = student_logits[:, -1, :]
    teacher_final = teacher_logits[:, -1, :]

    ce = F.cross_entropy(student_final, labels)
    kl = F.kl_div(
        F.log_softmax(student_final / T, dim=-1),
        F.softmax(teacher_final / T, dim=-1),
        reduction="batchmean"
    ) * (T ** 2)
    cos_sim = F.cosine_similarity(student_final, teacher_final, dim=-1)
    cosine_loss = 1 - cos_sim.mean()

    return ce_weight * ce + kl_weight * kl + cosine_weight * cosine_loss
...
def layered_consensus_process(question, agents, base_options, gold_answer):
    ...
    if consensus:
        ...
        student_input = tokenizer(question, return_tensors="pt").to(device)
        label_token = tokenizer.convert_tokens_to_ids(tokenizer(decision.lower()))
        label_tensor = torch.tensor([label_token], device=device)

        with torch.no_grad():
            teacher_logits = teacher_model(**teacher_input).logits
        student_logits = student_model(**student_input).logits

        loss = distill_final_token_loss(student_logits, teacher_logits, label_tensor)
        loss.backward()
        optimizer.step()
    ...
    if vote_totals:
        ...
        teacher_input = tokenizer(question, return_tensors="pt").to(device)
        student_input = tokenizer(question, return_tensors="pt").to(device)
        label_token = tokenizer.convert_tokens_to_ids(tokenizer(final_decision.lower()))
        label_tensor = torch.tensor([label_token], device=device)
